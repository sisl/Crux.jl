...
## Train individually
solve(DQNSolver(π = DQNPolicy(Q = Q(), actions = as), S = S, N = N, log = LoggerParams(dir = "log/ind_task1")), tasks[1])
solve(DQNSolver(π = DQNPolicy(Q = Q(), actions = as), S = S, N = N, log = LoggerParams(dir = "log/ind_task2")), tasks[2])
solve(DQNSolver(π = DQNPolicy(Q = Q(), actions = as), S = S, N = N, log = LoggerParams(dir = "log/ind_task3")), tasks[3])
...
## Train Jointly
𝒮_joint = DQNSolver(π = DQNPolicy(Q = Q(), actions = as), S = S, N = N, batch_size = 96, log = LoggerParams(dir = "log/joint"))
solve(𝒮_joint, tasks)
...
𝒮_seq = DQNSolver(π = DQNPolicy(Q = Q(), actions =as), S = S, N = Nsteps_per_cycle, 
...
𝒮_er = DQNSolver(π = DQNPolicy(Q = Q(), actions = as), S = S, N = Nsteps_per_cycle, 
                  exploration_policy = EpsGreedyPolicy(MultitaskDecaySchedule(Nsteps_per_cycle, 1:length(seq_tasks)), rng, as),
...
𝒮_ewc = DQNSolver(π = DQNPolicy(Q = Q(), actions = as), S = S, N = Nsteps_per_cycle, 
                  exploration_policy = EpsGreedyPolicy(MultitaskDecaySchedule(Nsteps_per_cycle, 1:length(seq_tasks)), rng, as),
