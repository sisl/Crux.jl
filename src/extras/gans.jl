...
Lᴰ(t::GAN_BCELoss, D, x, xtilde; wD = 1f0, yG = (), yD = ()) = LBCE(D(vcat(x, yD...)), wD) + LBCE(D(vcat(xtilde, yG...)), 0f0)
Lᴳ(t::GAN_BCELoss, G, D, z; yG = ()) = LBCE(D(vcat(G(vcat(z, yG...)), yG...)), 1f0)
...
Lᴰ(t::GAN_HingeLoss, D, x, xtilde; wD = 1f0, yG = (), yD = ()) = mean(relu.(1f0 .+ (-2f0 .* wD .+ 1f0) .* D(vcat(x, yD...)))) + mean(relu.(1f0 .+ D(vcat(xtilde, yG...))))
Lᴳ(t::GAN_HingeLoss, G, D, z; yG = ()) = -mean(D(vcat(G(vcat(z, yG...)), yG...))) 

## WLoss
struct GAN_WLoss <: GANLoss end
Lᴰ(t::GAN_WLoss, D, x, xtilde; wD = 1f0, yG = (), yD = ()) = mean(D(vcat(xtilde, yG...))) -  mean((wD .* 2f0 .- 1f0) .* D(vcat(x, yD...)))
Lᴳ(t::GAN_WLoss, G, D, z; yG = ()) = -mean(D(vcat(G(vcat(z, yG...)), yG...))) 
...
function Lᴰ(t::GAN_WLossGP, D, x, xtilde; wD = 1f0, yG = (), yD = ())
	loss = mean(D(vcat(xtilde, yG...))) -  mean((wD .* 2f0 .- 1f0) .* D(vcat(x, yD...)))
