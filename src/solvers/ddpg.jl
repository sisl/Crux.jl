@with_kw mutable struct DDPGSolver <: Solver
    π::DDPGPolicy # behavior policy
    S::AbstractSpace # state space
    A::AbstractSpace = action_space(π) # action space
    N::Int = 1000 # number of training iterations
    rng::AbstractRNG = Random.GLOBAL_RNG
    exploration_policy::ExplorationPolicy = GaussianNoiseExplorationPolicy(0.1f0, rng)
...
DDPG_target(π, 𝒟, γ::Float32) = 𝒟[:r] .+ γ .* (1.f0 .- 𝒟[:done]) .* target_value(π, 𝒟[:sp], target_action(π, 𝒟[:sp])) # DDPG
